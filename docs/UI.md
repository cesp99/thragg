...
- solana/build/BuildRunner.kt — the one entry point. It does, atomically and in this order: (1) save every dirty buffer in the project and wait for the writes; (2) for Anchor and Seahorse, reconcile the program id — since 2026-09-08 the app does this itself first (ProgramIds.syncProgramIds: it makes the keypair when there is none and writes declare_id!, Anchor.toml and a Seahorse program's Python), with `anchor keys sync` only as the fallback when lib.rs still holds the placeholder; (3) spawn the task through TaskRunner; (4) stream to the log; (5) parse; (6) publish. A second press while running is Stop, not a second build. Save-before-build is not optional and not a setting — the judges' 'edit, tap Build, read the error you already fixed' failure is designed out.
...
- ui/shell/build/BuildScreen.kt and RunDeck.kt — the destination host, the virtualized log, and the run deck: a 48dp row of keys directly above the nav capsule, order left→right Build, Test, Deploy, Shell. Build is twice as wide as Test or Deploy (it is the key that gets hammered); Shell is a square icon key that toggles the terminal in place of the old top-bar chip. A blocked verb keeps its label at full ink and loses only its fill; the reason (`Deploy · needs a build`, `no toolchain`, `open a project`) is read in the status strip's trailing slot, which flashes in the warning ink when the blocked key is tapped, and holding a blocked key opens what gates it (Deploy → Wallet, Build → Problems). While a run is going, Build becomes `■ Stop` with the elapsed clock inside it, the strip shows the spinner and the word, and the other keys go quiet — nothing repeats "building". The deck hides under the keyboard. (Chosen on the phone on 2026-09-08 from four stylings; the original had Test and Deploy in the ⋮ menu.)
...
- The log follows the tail until the user scrolls up, and a user scroll back to the bottom re-engages it; a new run always starts following, and re-tapping Build jumps to the end. Only a user's own scroll changes this — never a layout change, never the log's own catch-up (BuildLogView.kt, TailFollow.kt; measured 2026-09-08 when the first version stopped following without a touch).
- Test honesty. Anchor's scaffolded `[scripts] test` is `yarn run ts-mocha …`, so Node (with yarn) is an optional Setup row — the manifest's `node`, 57 MB. So: for Native, Test runs `cargo test` and works. For Anchor without Node, the Test press opens a sheet — 'Node is not installed' (or 'yarn is not installed') — with Set up, which pushes Setup, and 'Run cargo test instead' for the program's own Rust tests. With Node, Test runs `anchor test --skip-local-validator --skip-deploy`, after a one-time `yarn install` and after writing the deploy key to the wallet path Anchor.toml names: there is no Agave CLI in the guest, so it can neither start a validator nor deploy, and the tests hit the program Deploy already put on the selected cluster — the log says so before they run. None of this is hidden behind a button that just fails.
...
- Mainnet-beta gets a second, red confirm naming the program, the cluster and the SOL cost. Airdrop is not offered off devnet/testnet (there is no localnet: no validator runs on the phone).
...
Reached four ways, and the first two are the ones that matter: the Build overflow's Wallet item and the Deploy sheet's balance row ("Wallet", beside the deploy key's balance), because the Build tab is where SOL is spent — and from there the sheet opens with the DEPLOY KEY card first, "Mine 5 SOL" above the fold, two taps from the tab; then Projects & tools and Settings, where Seed Vault comes first as before. Until 2026-09-08 only the last two existed, and the deploy key's "Mine 5 SOL" was five taps from the Build tab (Code, Files, Projects, Wallet, scroll) — measured by looking for it. The Deploy sheet also says, under the balance, when the key is short and how the deploy will cover it, since the deployer mines or airdrops the difference by itself.
...
The gate has no text link under the button while the install runs. Once the required rows are in the button reads Continue; reached from Settings afterwards the same screen has "Check for updates", Close and "Remove the toolchain — frees N GB" (the bytes this phone actually holds). A check that finds rows behind the published manifest marks them "installed · update available" and turns the primary button into "Update (N MB)", which reinstalls only those rows (docs/SOLANA.md, "Updating the toolchain").

Once the toolchain is in and nothing is installing, one more row sits under the components in the same card: **Build cache**, the shared dependency cache's warm-up (docs/SOLANA.md, "Build cache" → Priming). It is drawn exactly like a component row and has four states: `not primed — first build compiles every dependency (about 5 min)` with a **Prime now** chip; `priming · Anchor · 2m10s` (the step and an elapsed that ticks with the install's clock, a bar under it) with **Stop**; `primed · 2026-09-19` with what the run took as its figure; `failed · <reason>` with **Retry**. The phone primes on its own after an install lands and on app start when the key is stale; the chip is for a user who does not want to wait and find out. A Build pressed while it primes wins: the row goes back to not primed and priming resumes after the build.
...
- Seahorse honesty: selecting it shows a one-line caption that Seahorse builds through `anchor build` and that its compiler is an optional Setup row (a two-minute on-device compile), and the Build tab names it — 'Seahorse is not installed', with Setup one tap away — rather than failing three lines into a log. The scaffold ships the crate manifest and a placeholder `lib.rs` Seahorse never writes, so the workspace loads before the first build. A framework you can create but cannot build is worse than one never offered.
...
- 'Edit settings.json' opens the real JSONC file in Code, so every engine key stays reachable for the person who wants lsp.rust-analyzer.initialization_options or a per-language formatter. It just has no row.
