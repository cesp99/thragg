...
MUST RESPECT REDUCE-MOTION: every item above; the transcript's follow-the-tail scroll (already, via `revealItem`); chevron rotation; card expand/collapse; the status strip's `animateContentSize`; the spinner (stands still rather than disappearing — it still marks "running"); the slider's *fill colour* animation.
...
1. `SliderDefaults.Track(trackCornerSize = 12.dp, …)` is internal. Use the public `Track(sliderState, modifier, enabled, colors, drawStopIndicator = null, drawTick = { off, c -> drawCircle(c, 2.5.dp.toPx(), off) }, thumbTrackGapSize = 8.dp, trackInsideCornerSize = 6.dp)`. The only loss is the outer corner radius — the track keeps its default full-pill ends instead of a 12dp square-ish end. The gap either side of the handle, the inside corners and the per-level dots — the three things that actually read as Expressive — are all there.
...
```
┌──────────────────────────────────────────────────────────────┐
│  seeker-ide                                     ⌄   ＋   ⋮   │ 56  TopAppBar
│  Spettro · coding                                            │     subtitle 12sp
├──────────────────────────────────────────────────────────────┤ 1dp outlineVariant
...
```kotlin
@Composable fun EmptyState(headline: String, body: String, modifier: Modifier = Modifier, action: (@Composable () -> Unit)? = null)
```
