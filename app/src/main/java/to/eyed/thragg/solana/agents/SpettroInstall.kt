...
import to.eyed.thragg.core.AgentDefinition
import to.eyed.thragg.core.Agents
...
object SpettroInstall {
    ...
    fun ensureRegistered(context: Context): Result {
        ...
        val settings = runCatching { AppSettings.load() }.getOrElse { error ->
            ...
            return Result.Failed("Thragg could not read its settings.")
        }
        ...
        val written = runCatching {
            ...
            null
        } ?: return Result.Failed("Thragg could not save the agent to its settings.")
    ...
    private fun probeAcpArgs(app: Context): Probe {
        ...
        val help = runCatching { runHelp(app) }.getOrElse { error ->
            ...
            null,
            "Thragg could not run the agent's own help, so it wrote the usual " +
        ...
        return Probe(
            ...
            "The agent's help did not mention ${entry.fallback.joinToString(" ")}, " +
                "which is the flag Thragg wrote anyway. If the first message fails " +
