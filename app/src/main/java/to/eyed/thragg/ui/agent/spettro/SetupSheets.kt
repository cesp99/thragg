...
import to.eyed.thragg.core.ConnectOutcome
import to.eyed.thragg.core.LocalProbe
import to.eyed.thragg.core.ProviderEntry
            ...
            chosen?.let { provider ->
                ...
                Text(
                    text = "Your key is verified with ${provider.name}, then stored " +
                        "encrypted on this device. Thragg never keeps a copy.",
                    style = MaterialTheme.typography.bodySmall,
                    color = MaterialTheme.colorScheme.onSurfaceVariant,
                )
...
fun LocalModelSheet(state: ShellState, onDismiss: () -> Unit) {
    ...
    SheetScaffold(
        ...
        Column(
            ...
            Text(
                text = "Ollama and LM Studio both speak the OpenAI API. Point Thragg " +
