        ...
        output.get().asFile.apply { parentFile.mkdirs() }.writeText(
            """
            {
              "note": [
                ...
                "Every Maven module on the RELEASE runtime classpath of both flavours —",
...
tasks.register<DumpMavenLicences>("dumpMavenLicences") {
    ...
    // Both flavours, because they differ in what they *can execute*, not in
    // what they depend on — and a module that reached only one of them would
    // otherwise be missing from the notices of the edition that ships it.
    coordinates.set(
        mavenCoordinates("fullReleaseRuntimeClasspath")
            .zip(mavenCoordinates("playReleaseRuntimeClasspath")) { full, play -> full + play }
    )
